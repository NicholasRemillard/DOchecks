...
# Onedrive
parent_folder <- c("C:/Users/nickr/OneDrive - University of Tennessee/PAAL Undergrad Docs/02 DO Coding/04 Annotated Participant Files/")

child_folder <- c("01 Exported Observer Excel Files")
...
setwd(paste(parent_folder, child_folder, sep=""))
...
select_video <- function(parent_folder, child_folder) {
  # Select file
  ...
  vid_name <- gsub(paste(parent_folder, child_folder, sep=""), "", comp_name)
...
video1_info <- select_video(parent_folder, child_folder)
...
if(compare_files_yesno == "yes"){ # If comparing two videos and doing quality control
  ...
  video2_info <- select_video(parent_folder, child_folder)
